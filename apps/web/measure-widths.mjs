...
for (const w of [700, 760, 800, 840]) {
  await page.evaluate((max) => {
    const chat = document.querySelector(".v3-chat");
    if (chat) chat.style.setProperty("--v3-chat-max", `${max}px`);
  }, w);
  await page.waitForTimeout(400);
  await page.screenshot({ path: `C:/Users/jmmas/AppData/Local/Temp/opencode/chat-w${w}.png` });
  const out = await page.evaluate(() => {
    ...
    const md = document.querySelector(".v3-msg--assistant .v3-msg__card .v3-md");
    return {
      column: info(".v3-chat__column"),
      card: info(".v3-msg--assistant .v3-msg__card"),
      md: md ? info(".v3-msg--assistant .v3-msg__card .v3-md") : null,
